module.exports = function makeAuthService(deps) {
  ...
  async function getUserByEmailPublic(email) {
    const q = await db.query('SELECT id, email, first_name, last_name, profile_picture, bio, location FROM users WHERE email = $1', [email]);
    return q.rowCount === 0 ? null : q.rows[0];
  }
  ...
  async function findRoleByName(name) {
    const q = await db.query('SELECT id FROM roles WHERE name=$1 LIMIT 1', [name]);
    return q.rowCount === 0 ? null : q.rows[0];
  }

  async function addUserRole(userId, roleId) {
    await db.query('INSERT INTO user_roles (user_id, role_id, created_at) VALUES ($1,$2,now()) ON CONFLICT (user_id, role_id) DO NOTHING', [userId, roleId]);
  }

  async function listRolesForUser(userId) {
    const q = await db.query(`SELECT r.name FROM roles r JOIN user_roles ur ON ur.role_id = r.id WHERE ur.user_id=$1`, [userId]);
