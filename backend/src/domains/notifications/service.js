module.exports = function createService(deps) {
	...
	async function createNotification({ userId, actorId = null, type, targetType = null, targetId = null, data = {} }) {
		try {
			...
			if (prefQ.rowCount > 0 && prefQ.rows[0].enabled === false) {
				(logger.info || console.info)('Notification skipped due to user preference', { userId, type });
				return null;
			}
			...
			(logger.error || console.error)('createNotification error', err);
			throw err;
		}
	}
	...
	return { getPreferences, setPreference, createNotification, listNotifications, markRead };
};
