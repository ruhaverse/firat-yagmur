module.exports = function createService(deps) {
  ...
  // Create a swap with transaction for atomic media inserts
  const createSwap = async (swap) => {
    ...
    try {
      ...
      if (Array.isArray(media_files) && media_files.length) {
        const inserts = media_files.map(m => client.query(
  ...
  // Update a swap with transaction for atomic media inserts
  ...
  const listSwapsByUser = async (email, { page = 1, limit = 20, status, location } = {}) => {
    ...
    const totalRes = await db.query(`SELECT COUNT(*) FROM swaps s JOIN users u ON s.user_id=u.id ${where}`, values);
    return {
      items: items.rows,
      pagination: { page, limit, total: parseInt(totalRes.rows[0].count, 10) }
    };
  ...
  // List saved swaps by user email
  const listSavedSwaps = async (email, { page = 1, limit = 20, status, location } = {}) => {
    ...
    const totalRes = await db.query(`SELECT COUNT(*) FROM swaps s JOIN saved_swaps ss ON s.id=ss.swap_id WHERE 1=1 ${where}`, values);
    return {
      items: items.rows,
      pagination: { page, limit, total: parseInt(totalRes.rows[0].count, 10) }
    };
  ...
  // Save a swap (pre-check added)
  const saveSwap = async (userId, swapId) => {
    const swapRes = await db.query(`SELECT id FROM swaps WHERE id=$1`, [swapId]);
    if (!swapRes.rows.length) throw new Error('Swap not found');
