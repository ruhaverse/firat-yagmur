...
function makeController(deps) {
  const authService = (deps.services && deps.services.authService)
    ? deps.services.authService
    : require('./service')(deps);
  const config = deps.config;
  const JWT_SECRET = config.jwtSecret;
  const SALT_ROUNDS = config.saltRounds || 10;
  ...
  function sanitizeInput(input) {
    return input && typeof input === 'string' ? input.trim().slice(0, 255) : '';
  }
  ...
  async function assignRoleToUser(req, res, next) {
    try {
      ...
      await authService.addUserRole(targetId, r.id);
