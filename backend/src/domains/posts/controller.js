const makeController = (deps) => {
  ...
  async function likeUnlike(req, res, next) {
    try {
      ...
      if (result.liked && result.authorId && result.authorId !== uid) {
        try {
          await notificationService.createNotification({ userId: result.authorId, actorId: uid, type: 'post_like', targetType: 'post', targetId: pid, data: { liker: uid, post_id: pid } });
  ...
  async function getPostComments(req, res, next) {
    try {
      ...
      const mapped = rows.map(r => ({ ...r, profile_picture: r.profile_picture ? makeFileUrl(r.profile_picture) : null }));
